...
num_df = X_train.select_dtypes(include=['float64','int'])
...
# Code starts here
cat_col = cat_df.columns.values.tolist()
for i in range(0, len(cat_col)):
    X_train[cat_col[i]].fillna('NA', inplace=True)
    le = LabelEncoder()
    X_train[cat_col[i]]= le.fit_transform(X_train[cat_col[i]])
    X_test[cat_col[i]].fillna('NA', inplace=True)
